...
public class WebScraper
{
    ...
    public void FillProductList(string uri)
    {
        try
        {
            var web = new HtmlWeb();
            ...
            if (nextLinkNode != null)
            {
                ...
                if (nextUrl != null)
                {
                    FillProductList("https://www.officemag.ru" + nextUrl + "/");
                }
            }
        }
        catch (Exception ex)
        ...
        try
        {
            ...
            using (var context = new DataBaseContext())
            {
                var productEntity = new ProductEntity
                {
                    ...
                    FeaturesHtml = product.FeaturesHtml,
                    GoodsId = int.Parse(uriNumbers),

                };

                productEntity.Images = product.ImageLinks.Select(link => new ImageEntity { ImageLink = link, GoodsId=int.Parse(uriNumbers) }).ToList();
...
class Program
{
    static async Task Main(string[] args)
    {
        static List<CategoryEntity> GetCatalogsWithGoods()
        {
            using (var context = new DataBaseContext())
            {
                var idCatalogs = context.Categories.Where(item => item.ItemsInCategory == true)
        ...
        //await scraper.ProcessProductPageAsync("https://www.officemag.ru/catalog/goods/143448/");
        //scraper.FillProductList("https://www.officemag.ru/catalog/897/");
        var lst = GetCatalogsWithGoods();

        foreach (var item in lst)
        {
            scraper.CategoryId = item.IdCatalog;
            scraper.FillProductList("https://www.officemag.ru" + item.Url);
